...
/**
 * wisej.web.ext.IgniteWidget
 *
 * Wraps all the basic functionality of Ignite.
 */
qx.Class.define("wisej.web.ext.IgniteWidget", {
	...
	properties: {
		 ...
		 *
		 * The name of the actual ig javascript class.
		 */
	...
	members: {
		...
		_registerEventHandlers: function (options) {
			...
			if (handlers && handlers.length > 0) {
				for (var i = 0; i < handlers.length; i++) {
					...
					options[name] = this._makeEventHandler(name, handler);
				}
			}
				...
				for (var i = 0; i < wiredEvents.length; i++) {
					for (var i = 0; i < wiredEvents.length; i++) {
						...
						if (options[name] === undefined) {
							options[name] = this._makeEventHandler(name, null);
						}
					}
				}
			}
		...
		_makeEventHandler: function (name, handler) {
			var me = this;
			var type = name;

			function callback(args, rawEventData) {

				rawEventData.container = me;
				// Adds the type of event to the response data
				rawEventData["type"] = type;

				if (handler)
					handler.call(this, rawEventData);

				var wiredEvents = me.getWidgetWiredEvents();
				if (wiredEvents && wiredEvents.indexOf(type) > -1) {

					setTimeout(function () {

						// make sure there is always one field or
						// the wisej system sends null instead of {}.
						var eventData = me.filterEventData(rawEventData);
